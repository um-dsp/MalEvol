...
def Enticement_source(file):
    ...
    conversation1=conversations[0]["uris"]
    referer=conversation1[0]["referer"]
    found=0
...
def check_redirect(chain,conversation1,Id,conv_Id,files):
    
    head=conversation1[Id]["res_head"]
    # HTTP Redirect
    if re.search('3\d\d', conversation1[Id]["res_num"]) != None:
        print('-- HTTP Redirection: ',conversation1[Id]["res_num"])
        if head != "":
            if head.find('Location') != -1:
                if conv_Id==0:
                    chain[-1]=chain[-1]+'/'+conversation1[Id]["filename"]
                chain.append(head[head.find('Location')+len('location')+2:])
        else:
            print('Checking the response head in the file ',conversation1[Id]["filename"])

    # js-based redirect
    elif re.search('.*.js', conversation1[Id]["filename"]) != None:
        print("-- Checking for js-based redirection in ",conversation1[Id]["filename"])
        for file in files:
            if file.find(conversation1[Id]["filename"]) != -1:
...
def find_next(chain,conv_Id,conversations,files):
    conversation1=conversations[conv_Id]["uris"]
    for Id in range(len(conversation1)):
        if conv_Id==0:
            chain=check_redirect(chain,conversation1,Id,conv_Id,files)
        elif chain[-1].find(conversation1[Id]["filename"]) != -1: 
            chain=check_redirect(chain,conversation1,Id,conv_Id,files)
    return chain
